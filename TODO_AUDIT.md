...
- **Statut : EN COURS (tranche 1 : Personnel, tranche 2 : Objectif de saison, tranche 3 : Analyse adversaire, tranche 4 : Prêts, tranche 5 : Contrats, tranche 6 : Équipe B, tranche 7 : Transferts national — voir constat de risque et tranches suivantes ci-dessous)**
...
**Domaines restants (tranches suivantes possibles, non commencées).** Transferts internationaux (`approcherJoueurAdverse`/`convertirJoueurAdverseEnEffectifEtendu`, nécessite d'exporter une fonction dédiée de resynchronisation de `compteurJoueurId` comme en tranche 1), génération de base des joueurs/effectifs, génération de club/pyramide, composition/tactique, fatigue/moral/entraînement/blessures, calendrier/classement, sauvegarde/migration — chacun avec ses propres dépendances croisées à vérifier avant de couper, comme pour les tranches précédentes. `clubUI.js` (rendu) n'a pas encore été commencé.
