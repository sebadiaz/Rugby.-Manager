...
- `docs/js/club*.js` — **le Mode Club**, devenu la plus grosse partie du projet (42 modules, ~16 800 lignes). Chacun est une IIFE `(function (global) {…})(window)` qui fusionne ses fonctions sur `window.RMClub` via `Object.assign`, jamais par réaffectation : l'ordre des `<script>` n'a donc pas d'importance. `club.js` porte le modèle et les règles partagées (joueur, salaire, valeur de transfert, vieillissement, trésorerie) ; les autres couvrent un domaine chacun — temps et calendrier, composition, tactique, entraînement, médical, contrats et négociations, marché des transferts, finances et grand livre, direction, carrière du manager, compétitions, monde.
- `docs/js/clubUI.js` — l'interface du Mode Club : navigation par onglets, écrans, fiche joueur, résolution des journées. Fichier volumineux et fortement couplé à son état (`saison`, contexte d'équipe, rafraîchissements) ; il n'a pas de frontière d'extraction propre en l'état, ce qui est documenté comme chantier à part.
- `docs/js/club-sauvegarde.js` — persistance et **migrations versionnées** (`VERSION_SAUVEGARDE`) : une carrière commencée sur une version antérieure reste chargeable, chaque migration étant testée.
...
- `server/test-*.js` — **29 suites**, à lancer depuis la racine (`node server/test-<nom>.js`). Trois familles : le **moteur de match** (invariants, ruck, touche, mêlée, équilibre, feuille de match), le **Mode Club** (parcours complet de carrière, contrats et transferts, marché vivant, économie, pyramide des âges, comptes, infrastructures, médical, composition, direction), et le **parcours navigateur** (`test-parcours-navigateur.js`, qui pilote réellement Chromium sur les 16 onglets, en desktop et en mobile). Aucune n'a besoin d'installation : elles chargent les modules du dépôt tels quels.
